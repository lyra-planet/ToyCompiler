...
function Compiler() {
  ...
  const onLexingClick = (e) => {
    try {
      ...
      setIr(flat(Object.values(evaluator.irFuncArray).map(item => item.ir)))
    } catch (e) {
      console.log(e)
    }
  }
  ...
  return (
    <div className="Compiler h-screen flex overflow-hidden">
      <div className='w-1/2 h-full'>
        ...
        <div className='h-1/2 w-full'>
          <h3 className=' text-blue-700 bg-blue-200 p-2'>AST</h3>
          <div className='p-2 h-full w-full'>
            <textarea readOnly defaultValue={ir.join('\n')} className='resize-none h-5/6 w-full border-2 border-gray-400 rounded-md'
            />
          </div>
        </div>
      </div>
